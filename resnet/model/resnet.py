...
class ResNet(nn.Module):
    def __init__(self, block: Type[Union[BasicBlock, BottleneckBlock]], blocks: List[int], filters: List[int],
                 num_classes: int, zero_padding: Optional[bool] = False) -> None:
        super(ResNet, self).__init__()

        assert len(blocks) == 3 or len(
            blocks) == 4, "ResNet can either have `3` blocks for CIFAR10, or `4` for ImageNet"
        ...
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=filters[0], kernel_size=kernel_size, stride=stride,
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, BottleneckBlock]], num_blocks: int, planes: int,
...
def resnet44(zero_padding: bool = True, pretrained: bool = False) -> ResNet:
    ...
    if pretrained:
        url = "https://github.com/alvarobartt/understanding-resnet/releases/download/v0.1-cifar10/resnet44a-cifar10.pth" \
            if zero_padding else "https://github.com/alvarobartt/understanding-resnet/releases/download/v0.1-cifar10/resnet44b-cifar10.pth"
        model.load_state_dict(load_state_dict_from_url(url))
    return model
